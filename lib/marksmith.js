    ...
    fs.stat(item.filename, function(err, stat) {
        ...
        if ( stat.isDirectory() ) {
            ...
            fs.readFile(item.filename, 'utf8', function(err, data) {
                // data from the file
                // store[url].filename = item.filename;
                // store[url].data = data;
...
function pluginSetSlugs(store) {
    fmt.line();
    fmt.title('Plugin : Set Slugs');

    var urls = Object.keys(store);

    urls.forEach(function(url, i) {
        fmt.field('SetsSlugs', url);
        store[url].slug = url;
        store[url].url  = url;
    });
}
...
module.exports = function(dir, done) {
    ...
    queue.drain = function() {
        ...
        // set the slugs on all items
        pluginSetSlugs(store);

        // make the Gravatar bits from the email address
        pluginMakeAuthorGravatarFromEmail(store);

        // make the Gravatar bits from the email address
        pluginMakeGravatarUrlSizes([ 24, 32, 64, 128 ])(store);
